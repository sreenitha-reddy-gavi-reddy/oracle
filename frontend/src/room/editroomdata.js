...
const EditroomInfo = () => {
    ...
    console.log(id);
    useEffect(() => {
        axios.get('http://168.138.138.67:5000/api/getsingle-user/' + id).then((res) => { console.log(res.data.response)
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        axios.put(`http://168.138.138.67:1000/api/updateuser/${id}`, formdata)
            .then((res) => { 
                alert("Updated Sucessfully"); 
                console.log(res.data);
                window.location.replace(
                   "http://168.138.138.67:3001/room/getroomdata"
                  );
            })
            .catch((error) => {
